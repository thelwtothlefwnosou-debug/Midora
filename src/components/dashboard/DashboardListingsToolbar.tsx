...
const TABS: { id: ListingFilterTab; label: string }[] = [
  { id: "all", label: "Όλες" },
  { id: "published", label: "Δημοσιευμένες" },
...
const SORT_OPTIONS: { id: ListingSortOption; label: string }[] = [
  { id: "recent", label: "Πρόσφατες αλλαγές" },
  { id: "views", label: "Περισσότερες προβολές" },
  { id: "inquiries", label: "Περισσότερα αιτήματα" },
...
export function DashboardListingsToolbar({
  ...
  const visibleTabs = useMemo(() => {
    ...
    return TABS.filter((t) => t.id === "all" || tabCounts[t.id] > 0 || tab === t.id);
        ...
        {visibleTabs.map((item) => {
          ...
          return (
            ...
            >
              {item.label}
              {count > 0 ? (
                <span className={cn("ml-1 tabular-nums", active ? "text-white/80" : "text-muted")}>
                  {count}
                </span>
              ) : null}
            </button>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          <label className="inline-flex h-9 items-center gap-1 rounded-lg border border-border bg-white px-2 text-xs">
            <span className="text-muted">Τύπος</span>
            <select
              value={rentalFilter}
              onChange={(e) =>
            ...
            >
              ...
              <option value="short_term">Βραχυχρόνια</option>
          ...
          <label className="inline-flex h-9 items-center gap-1 rounded-lg border border-border bg-white px-2 text-xs">
            <span className="text-muted">Ταξινόμηση</span>
            <select
              value={sort}
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-white px-6 py-14 text-center">
      ...
      <p className="mt-4 font-display text-lg font-semibold text-charcoal">
        Δεν βρέθηκαν αγγελίες σε αυτή την κατηγορία
      </p>
      <p className="mt-1 text-sm text-muted">Δοκίμασε άλλο φίλτρο ή αναζήτηση.</p>
      {tab !== "all" && onShowAll && (
        <button
          type="button"
          onClick={onShowAll}

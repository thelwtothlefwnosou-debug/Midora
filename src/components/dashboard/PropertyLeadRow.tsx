...
import { leadDurationKey, leadStatusKey } from "@/lib/lead-labels";
...
function listingCoverUrl(listing: NonNullable<PropertyLeadWithListing["listings"]>): string | null {
  ...
  return pickListingCoverPhotoUrl(listing);
...
function formatInterestDates(

  lead: PropertyLeadWithListing,

  locale: string,

  t: ReturnType<typeof useTranslations<"Owner.leads">>

): string | null {

  if (lead.interest_start_date && lead.interest_end_date) {

    const start = safeFormatDate(lead.interest_start_date, locale, { dateStyle: "medium" });

    const end = safeFormatDate(lead.interest_end_date, locale, { dateStyle: "medium" });
  ...
  if (lead.interest_start_month) {
    ...
    if (Number.isFinite(y) && Number.isFinite(m) && m >= 1 && m <= 12) {
      ...
      if (lead.interest_duration_months) {
        ...
        return `${label} · ${t("months", { count: lead.interest_duration_months })}`;
  ...
  if (lead.start_date) {
    ...
    return safeFormatDate(lead.start_date, locale, { dateStyle: "medium" });
...
export function PropertyLeadRow({
  ...
  const t = useTranslations("Owner.leads");
  ...
  const interestDates = formatInterestDates(lead, locale, t);



  const durationKey = leadDurationKey(lead.duration);

  const durationLabel = durationKey ? t(durationKey) : lead.duration;
  ...
  function replySenderLabel(reply: PropertyLeadReply): string {
    ...
    if (reply.sender_role === "cohost") {
      ...
      return t("replyCohost", { name: reply.sender_display_name });

    }

    return t("replyOwner", { name: reply.sender_display_name });
  ...
  const created =
    ...
    safeFormatDate(lead.created_at, locale, {
    ...
    <article className="overflow-hidden rounded-2xl border border-border bg-white shadow-soft">
      ...
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="min-w-0 flex-1 p-5">
          ...
          <div className="mt-4 grid gap-2 text-sm text-charcoal/75 sm:grid-cols-2">
            ...
            {lead.guests != null && lead.guests > 0 && (
              ...
              <p className="flex items-center gap-2">
                ...
                {lead.guests} {lead.guests === 1 ? t("person") : t("people")}
            ...
            {lead.duration && (
              ...
              <p className="flex items-center gap-2">
                ...
                {t("estimatedDuration", { duration: durationLabel ?? lead.duration })}
          ...
          <div className="mt-4 flex items-start gap-2 rounded-xl bg-sand/30 px-3.5 py-2.5 text-xs leading-relaxed text-charcoal/65">

            <Shield className="mt-0.5 h-3.5 w-3.5 shrink-0 text-gold-dark" aria-hidden />

            <p>{tLegal("threadSafety")}</p>
          ...
          <p className="mt-3 text-xs text-muted">{t("received", { date: created })}</p>

...
export function ListingAmenitiesSection({
  ...
  const grouped = amenitiesByCategory(keys);
        ...
        <div className="fixed inset-0 z-[180] flex items-end justify-center bg-charcoal/50 p-4 sm:items-center">
          <div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-center justify-between">
              <h3 className="listing-section-title text-lg">Παροχές</h3>
              <button
                type="button"
            ...
            <div className="mt-4 space-y-5">
              {([...grouped.entries()] as [AmenityCategory, string[]][]).map(
                ([cat, labels]) => (
                  <div key={cat}>
            ...
            {(grouped.has("safety") || grouped.has("accessibility")) && (
              <p className="mt-4 text-xs text-muted">
                Οι παροχές ασφάλειας και προσβασιμότητας δηλώνονται από τον αγγελιοδότη και
                δεν αποτελούν επίσημη πιστοποίηση.
              </p>
            )}
          </div>
        </div>

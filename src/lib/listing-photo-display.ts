...
import { isPhotoRoomKey, photoRoomLabel, type PhotoRoomDef } from "@/lib/photo-rooms-catalog";
...
export const PHOTO_GALLERY_FILTERS: {
  ...
  { id: "kitchen", label: "Κουζίνα", keys: ["kitchen"] },
  {
    id: "bathrooms",
    label: "Μπάνια",
...
export function roomBadgeLabel(roomKey: string | null | undefined): string | null {
  if (!roomKey || !isPhotoRoomKey(roomKey)) return null;
  return photoRoomLabel(roomKey);
}
...
export function firstFiveRoomWarning(photos: ListingImage[]): string | null {
  ...
  return "Πρόσθεσε στις πρώτες φωτογραφίες σαλόνι, υπνοδωμάτιο, κουζίνα ή μπάνιο για καλύτερη παρουσίαση.";
}
